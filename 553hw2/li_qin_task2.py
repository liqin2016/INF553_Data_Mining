...
dataFile3 = dataFile2.groupByKey().map(lambda x: list(x[1])).filter(lambda x: len(x) > filterNum)
...
def pass2(chunk):
    ...
    for chunkindex in range(0, len(chunk)):
        chunk[chunkindex] = frozenset(chunk[chunkindex])
